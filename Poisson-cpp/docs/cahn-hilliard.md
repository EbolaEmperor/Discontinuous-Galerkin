...
无通量边界是**自然边界**,内罚只作用于内部边,所以**任意形状的区域都能直接用**,无需改动求解器、质量守恒
与能量耗散照旧成立。`domain` 可选三种(中心 (0.5,0.5),特征尺寸 ~1,与单位正方形可比,故 $\varepsilon$ 等参数含义不变):

- **`"square"`**:单位正方形 $[0,1]^2$,结构化三角网(`Mesh::getMesh`)。
- **`"hexagon"`**:正六边形(外接半径 `domain_radius`),6 个等边三角形扇形细分到边长 $\le h$(`Mesh::getHexagonMesh`)。
- **`"disk"`**(别名 `"circle"`):圆盘,`Mesh::getDiskMesh` 自动按 $h$ 生成**高质量网格**——先建上面的六边形等边网格,再**保角度、按 $R/r_{\text{hex}}(\theta)$ 缩放半径**把六边形光滑映射到圆(正 Jacobian、无翻转;畸变 $\le15\%$ 且均摊到整个半径),边界精确落在圆上。

> 收敛测试 `cahn_hilliard_convergence` 仍固定在正方形(其制造解只在方形上满足无通量)。
...
|---|---|---|---|---|---|
| `domain` | 计算区域(§2.3) | string | `"square"` | `"square"` | `"square"` / `"hexagon"` / `"disk"`(别名 `"circle"`) |
| `domain_radius` | 六边形外接半径 / 圆半径 | double | 0.5 | 0.5 | 中心固定 (0.5,0.5);`square` 忽略 |
| `ord` | 多项式次数 $P_k$ | int | 1 | 1 | $\ge1$;渲染用顶点值,P1 最快 |
| `N` | 分辨率:元尺寸 $h=1/N$ | int | 128 | 64 | $\ge1$;越大越精细也越慢($\sim N^2$ 自由度) |
