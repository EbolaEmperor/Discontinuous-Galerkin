...
for cycle = 1 : Nref
    ...
    K = assembleK_Poi1D(fem, grid);
    P = assembleIP_Poi1D(fem, grid, sigma, beta);
    A = K + P;

    F = assembleLoad_DPk1D(fem, grid, f);

    % Nitsche weak Dirichlet BDC
    [Ahat, Fhat] = assembleWeakBdry_Poi1D(fem, grid, sigma, beta, u_exact(x0), u_exact(x1));
    A = A + Ahat;
    F = F + Fhat;
    sol = A \ F;
    ...
    [errH1(cycle), errL2(cycle), ~] = ...
        getH1Err_DPk1D(fem, grid, sol, u_exact, du_exact);
...
opt.nPoint = 20;
opt.u_exact = u_exact;
plotSol_DPk1D(fem, grid, sol, opt);
