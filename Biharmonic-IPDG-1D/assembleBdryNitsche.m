...
idx = 1:k+1;
B(idx,idx) = - d2phi' * dphi - dphi' * d2phi + (sigma/h0) * (dphi' * dphi);
F(idx) = - d2phi * g0 + (sigma/h0) * dphi * g0;
...
idx = (NT-1)*k + (1:k+1);
B(idx,idx) = - d2phi' * dphi - dphi' * d2phi + (sigma/h1) * (dphi' * dphi);
F(idx) = - d2phi * g1 + (sigma/h1) * dphi * g1;
