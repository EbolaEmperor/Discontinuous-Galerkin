...
| `euler_shock_bubble` | 域 $[0,2.5]\times[0,1]$，马赫 $1.22$ 激波，圆形密度跳变，**特征型右出流** | 单 $\gamma$ 代理。默认**重气泡** $\rho_b/\rho{=}3$（聚焦+射流，鲁棒）；轻气泡 $0.138$（涡环）更刚 |
| `euler_rmi_amr` | 域 $[0,4]\times[0,1]$，马赫 $1.5$ 激波冲过 $\rho_L{=}1/\rho_H{=}3$ 扰动界面（左移到 $x_0{=}0.7$），右壁透射出流 | Richtmyer–Meshkov：脉冲驱动（**无需重力**），蘑菇钉 + KH 卷起；`reshock` 开右壁反射→二次冲击湍流混合 |
| `euler_corner_diffraction` | L 形域 $([0,1]\times[6,11])\cup([1,13]\times[0,11])$，马赫 $5.09$ 激波 | 绕凸角衍射：弯曲衍射激波 + 角点大涡 + **角点近真空**（保正招牌） |

**人工粘性传感器与算例类型（关键、实测）**。双马赫反射是**激波主导**：用**压力传感器**（`av_indicator=1`），
人工粘性只作用激波、不碰接触/滑移线，故涡街锐利——**角点衍射**同属激波主导，沿用之（角点近真空交给
Zhang–Shu 保正）。但**接触/剪切主导**的算例（二维黎曼的滑移线、激波-气泡的密度界面、**RMI 的界面**）不同：
Zhang–Shu 保正只保正、不抑制**保持正值**的高阶振荡，而本求解器**无斜率限制器**（`tvbLimit` 为空操作），
故欠分辨的剪切层会**无界增长**（$\rho\to10^5$–$10^6$ 后 NaN）。对策：

- **二维黎曼 / RMI**：**密度传感器** + **低 Persson 阈值** `av_s0=-3`（及早起粘）+ `av_c=2`、`cfl\le0.4`。
  （单换 Rusanov 通量**无效**——及早起粘的阈值才是关键。RMI 的密度适中，故激进阈值不会破坏 SIPG 强制性。）
- **激波-气泡**：密度传感器但**温和**（默认 Persson 阈值、`av_c≈1.2`）。过激的低 `av_s0` 会在接触附近过度起粘，
  使 $\{\varepsilon\}$-加权 SIPG 在 `sigma_ip=20` 下丧失强制性（**反扩散→更快发散**）。默认**重气泡**（声速低、稳健）。
  另有 `cfl_rho_floor`：仅给 dt 的波速估计设密度地板，防虚假近真空节点把 $|u|+c$ 冲爆、压崩 dt（不碰物理）。

**二维黎曼边界条件**。自相似结构随 $x/t$ 增长，居中交点（`cross=0.5`）只能跑到标准短时（config 3 约 t=0.3）。
默认改用 **Schulz-Rinne 离心交点 `cross=0.8`**：主结构往左下大区域发展，固定域也能干净跑到 **t=0.8**。边界用
**弱远场鬼态**（恒取该侧象限常状态、经 HLLC 逐特征处理）：超声速出流忽略鬼态、亚声速混入远场、超声速入流全施加——
既修掉超声速入流壁（左/下）的伪侵入，也钉住亚声速剪切出流壁（右）的入射声学特征，消除长时间的伪卷起。

**激波-气泡右出流**。入射激波抵右壁后，后激波出流是**亚声速**（$u\sim0.4<c\sim1.26$）；裸零梯度会反射出向左生长的
**稀疏波黑楔**最终吞域 NaN，固定背压也会在另一态注入伪波。正解是**特征型非反射出流**：出射不变量（熵、切向速度、
$R^+$）取内部，入射 $R^-$ 取环境远场，重构鬼态——自适应环境/后激波两态、钉住 $R^-$ 不让压力漂移。再把气泡整体
**左移**（$c_x{=}0.4$、激波 $0.15$）给**慢**的重碎片腾出空间，固定域 $[0,2.5]$ 干净跑到 **t≈4.8**（碎片 ~5.2 才到右壁）。

**RMI 边界与时长上限**。界面/激波左移（$x_0{=}0.7$、激波 $0.3$）让结构有空间往右扫，`max_gen=6` 把界面/激波加密到
$h\approx1/280$。固定域 $[0,4]$ **干净到 t≈3.5**：之后透射激波（在重气中、出流处**亚声速** $u\sim0.82<c\sim0.92$）
抵右壁，零梯度出流长稀疏波楔——$\rho_\max$ 从 ~7.8 越过 3.5 后冲到上千、网格随之爆涨。**与气泡不同，左移救不了 RMI 的更长时长**：
气泡是局部慢碎片（左移即可腾空间），RMI 是以 ~0.82 横扫整个域的界面，任何起始位置在 t=5+ 都已扫出 $[0,4]$——要更久必须**加宽域**。

**L 形域**（corner）由 `makeMaskedRectMesh` 从整块 `makeRectMesh` **整格删去**孔洞单元再压缩顶点生成；孔洞边界
对齐网格线（`base_cell` 自动吸附到 $1/k$），删除整个单元对后剩余仍是合法交叉网格，自适应森林的最长边标号与协调闭合不受影响。

运行命令见 README §8.1；各算例参数见 `examples/{riemann,shock_bubble,rmi,corner}_config.json`。
