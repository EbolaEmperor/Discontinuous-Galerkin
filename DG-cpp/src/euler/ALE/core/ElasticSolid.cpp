...
double SolidALEMap::influence(double x, double y, double distance) const {
    ...
    double wall = smooth01(xWall / wallMargin_) * smooth01(yWallTop / wallMargin_);
    return near * wall;
}
...
Vector2d SolidALEMap::refToPhys(const Vector2d& Xp, double time) const {
    ...
    double w = influence(Xp.x(), Xp.y(), std::sqrt(d2));
    return Xp + w * (cur - ref);
}
...
Vector2d SolidALEMap::velocityAt(double x, double y, double time) const {
    ...
    double d2 = 0.0;
    double a = alpha(time);
    int idx = closestCurrentSegment(p, a, s, d2);
    ...
    double w = influence(x, y, std::sqrt(d2));
    return w * vel;
}
