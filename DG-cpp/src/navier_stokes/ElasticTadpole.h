#ifndef ELASTIC_TADPOLE_H
#define ELASTIC_TADPOLE_H

// ===========================================================================
// 2-D tadpole with a RIGID circular head and an ELASTIC immersed tail.
//
...
//   * Tail : a Cosserat elastic rod whose first two nodes are clamped to the
//            moving rear surface of the head.  The clamp carries the head's
//            actual material velocity, so the tail is a moving elastic boundary
//            rather than a line that is teleported into place every step.
//
// The driver treats the tail as an immersed elastic boundary: the fluid
// velocity is interpolated to the rod, and the rod's penalty/direct-forcing
// load is scattered back to the DG Navier-Stokes RHS through the adjoint
// IBCoupler operators.
// ===========================================================================
...
struct ElasticTadpole {
    ...
    double kRefuge   = 0.0;      // optional -k * grad |u|^2 wake-refuge force
    ...
    double dampAng   = 10.0;     // angular damping
    double maxSpeed  = 1.5;      // safety speed cap
    double maxOmega  = 0.5;      // safety angular-speed cap
    double maxOmegaAccel = 1.0;  // safety angular-acceleration cap
    ...
    // Update the two clamped tail nodes from the current head pose and velocity.
    void syncTailClamp();
    ...
    // Advance head + tail by dt against the live DG flow field.  The tail-fluid
    // drag is implicit in the rod solve.  Returns false on a degenerate sample.
